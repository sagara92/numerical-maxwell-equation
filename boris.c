...
static const double tfinal = 17.0;
...
static const double electric_field[3]   = {0.1, 0.0, 0.0};
...
void err_phase(double x[3], double phase)
{
    phase = atan(x[1] / x[0]);
}
...
int main()
{
    ...
    double omega_B = B_mag * partq / partm;
    ...
    while (t < tfinal)
    {
        fprintf(outfile, "%f %f %f %f %f %f\n", x[0], x[1], x[2], t, energy_diff, phase);//Writes the x, y, and z coordinates as well as the %error in energy and phase.
