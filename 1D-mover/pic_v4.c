...
static const double electric_field[4] = {0.0, 0.0, 1.0, 0.0};
...
void mag_int(double* E, double* B, double dt, double* Bnew)
{
    ...
    for (int i = 0; i < NUM_CELLS; ++i)
    {
        ...
        dEdx[i] = (Eplus - Eminus) / dx;
        Bnew[i] = B[i] - c * dEdx[i] * dt / 2.0;                                         //Step 6//
    } // B := B^{n}
}
...
void collocation_E_B(double* E, double* B, double x[4], double Enew[4], double Bnew[4])       //Step 7//
{
    ...
    double Bplus  = B[j];
    double Bminus = B[(j - 1 + NUM_CELLS) % NUM_CELLS];

    Bnew[3] = Bminus + (x[1] - domain_x0 - ((j - 1) * dx)) * (Bplus - Bminus) / dx;
    Bnew[0] = 0.0;
    Bnew[1] = 0.0;
    Bnew[2] = 0.0;
}
...
// ============================================================================
void push_particle(double E[4], double B[4], double x[4], double u[4], double dt)
{
    double t[4], s[4];
...
int main()
{
    ...
    FILE* outP = fopen("Particle_data.bin", "wb");
