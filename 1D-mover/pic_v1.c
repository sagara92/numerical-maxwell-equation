...
// ============================================================================
void mag_int(double* E, double* B, double dt, double Bnew[NUM_CELLS])
{
    // B := B^{n-1/2}
...
// ============================================================================
void collocation_E_B(double* E, double* B, double* x, double position[3], double Enew[4], double Bnew[4])       //Step 7//
{
    double dx = grid_spacing;
    int i = (int) position[1] / dx;
    int j = (int) (position[1] + 0.5) / dx;
        double Eplus  = E[(i + 1) % NUM_CELLS];
        double Eminus = E[i];
        Enew[2] = Eminus + (position[1] - x[i]) * (Eplus - Eminus) / dx;
        Enew[0] = 0.0;
        Enew[1] = 0.0;
        Enew[3] = 0.0;
        ...
        double Bplus  = B[j + 1];
        double Bminus = B[(j + NUM_CELLS) % NUM_CELLS];
        Bnew[3] = Bminus + (position[1] - x[j]) * (Bplus - Bminus) / dx;
        Bnew[0] = 0.0;
        Bnew[1] = 0.0;
        Bnew[2] = 0.0;
}
...
void exact_solution(double* E, double* B, double t, double dt)
{
    ...
    double w  = k * c;
    double dx = grid_spacing;
    double E0 = dot(electric_field, electric_field);
    double B0 = dot(magnetic_field, magnetic_field);
...
int main()
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        position[i] = state.x[i];
    }
    exact_solution(E, B, state.x[0], dt);
    FILE* outE = fopen("Electric1.bin", "wb");
    FILE* outB = fopen("Magnetic1.bin", "wb");
    FILE* outP = fopen("Particle_data1.bin", "wb");
    while (state.x[0] < tfinal)
    {
        mag_int(E, B, dt, Bn);
        collocation_E_B(E, Bn, state.x, position, E1, B1);
        state = advance(state, E1, B1);
        size_t elements_written_E = fwrite(&E1, sizeof(E1), 1, outE);
        size_t elements_written_B = fwrite(&B1, sizeof(B1), 1, outB);
