...
static const double wavenumber_k = 2.0 * M_PI / (domain_x1 - domain_x0);
static const double grid_spacing = (domain_x1 - domain_x0) / NUM_CELLS;
...
void output_solution(double* E, double* B, double t, double dt, int iteration)
{
    if (iteration % 100 == 0)
    {
        double dx = grid_spacing;
        char fname[1024];
        snprintf(fname, 1024, "output.%04d.dat", iteration / 100);
