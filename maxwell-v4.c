...
int main()
{
	double x, dx, xi, y, yi, xf, E0, B0, k, ti, w, c, tf, dt, t;
	...
	for (int i = 0; i < NUM_CELLS; ++i)
	{
		...
		L2_E[i] = sqrt(Diff_E[i]*Diff_E[i]);			
		Diff_B[i] = B_r[i] - B[i];
		L2_B[i] = sqrt(Diff_B[i]*Diff_B[i]);
	}
	...
	// Output to a file
    // ------------------------------------------------------------------------
    FILE* outfile = fopen("em-wave-v4.dat", "w");

    for (int i = 1; i < NUM_CELLS-1; ++i)
    {
        double x = xi + i * dx;
        fprintf(outfile, "%f %f %f %f %f %f %f\n", x, E[i], E_r[i], L2_E[i], B[i], B_r[i], L2_B[i]);
    }
